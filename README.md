...
**114일, 114개 실험 — Claude Code / MCP / GitHub Actions로 Android 개발 워크플로우를 자동화한 실험 기록**
...
→ [7차 실험 (091~104) — 같은 증상, 다른 원인](./PORTFOLIO_v7.md)
...
| 105 | [7차 실험 총정리 및 포트폴리오 v7 작성](./105-summary-v7) | 091~104 종합 정리, "같은 결과도 원인은 매번 다르다"는 7차 사이클 완성 |
| 106 | [알림 딥링크 버그 수정 — 101/103/104일차와 다른 계열의 원인](./106-notification-deeplink-fix) | implicit Intent와 intent-filter path 불일치가 원인, 104일차 이후 한 번도 안 돌던 계측 테스트를 같은 세션에서 발견·수정까지 완결 |
| 107 | [RAG 재인덱싱 — 인프라 사고 5연발과 함정의 세 번째 재현](./107-rag-reindex-saga) | core/ 인덱스 사각지대 발견, 다섯 번의 시도 끝에 Gradle 우회로 완주, 079/085일차 "유사도≠랭킹" 함정이 judge에 의해 다시 적발 |
...
| 114 | [Compose 동적 색상 배선 마무리 — XML만 진짜 디버깅이 필요했다](./114-compose-dynamic-color) | 113일차 함정 2가지가 Compose 구조상 애초에 성립 안 됨을 확인, 순수 함수 분리로 첫 유닛 테스트 추가 |
